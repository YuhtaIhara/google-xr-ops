...
> form 実物 4 page の各項目への英文 draft。 個人情報は 2026-05-21 session で user 確定済 (Q1-Q8)。 住所のみ機密扱いで `[REDACTED]` 維持、 user が form 提出時に直接入力。 提出言語は英語、 日本語 reference を併記。
...
form-structure-2026.md Page 1 全 7 項目。 spec § 9 Developer Readiness 上段の表と整合。 個人情報は user 確定済、 住所のみ機密扱いで `[REDACTED]` 維持。
...
日本在住のソロ indie 開発者。 設計・実装・QA を一人で担う。 共同創業者・チームなし。

Kotlin / Android 触れて 1 年未満。 採択後 9 ヶ月の development window を Jetpack XR SDK (DP4 → Beta) と Gemini multimodal API の習得 + Spice Sutra MVP 構築に充てる前提。 言語経験の浅さを補う leverage は、 既に完了している設計 (spec + plan、 添付 supporting files 参照)。

XR 経験: EVEN Realities G2 display glasses を 2026 年 3 月から毎日使用 (約 2-3 ヶ月)、 主に Conversate (G2 の flagship 会話 HUD use case)。 HUD UX を日常で体感する中で、 G2 にカメラが無いため「レシピ表示はできても工程判定ができない」 という限界を実感、 その gap が Spice Sutra の core となる。

Android 経験: Google Play への公開実績なし。 GitHub: github.com/YuhtaIhara、 public は SharePoint RAG (Azure 設計 / pgvector + FastAPI 実装)、 Spice Sutra の設計は `google-xr-ops` で公開済。 pinned は「設計 spec + clean structure + solo-maintainable scope」 で構成。

ドメイン知識: 自宅でインド・スパイス料理を日常的に調理。 parallel 調理 + 濡れた手 + スパイス手という問題を自分自身で体感しており、 自分が第一の検証者。

**根拠:**

form-structure-2026.md Page 2「Previous development experience」(必須・長文)。 spec § 9 Developer Readiness 表の Solo indie developer / G2 経験 / dogfood guarantee 各行が対応。 個人情報 / 公開実績は user 確定済 (2026-05-21)。
...
- `ai-driven-template` — a personal AI-assisted development starter template (`.claude/` configuration patterns)
...
ポートフォリオ: github.com/YuhtaIhara。 Google Play への公開実績なし、 Spice Sutra が初 Play release。 public repo: `google-xr-ops` (本応募の設計 + plan 公開)、 `ai-driven-template` (`.claude/` 設計パターン)、 `sharepoint-rag-azure` (Azure RAG PoC + IaC + 設計図)、 `sharepoint-rag-lite` (pgvector + FastAPI + ACL 実装)。

**根拠:**

form-structure-2026.md Page 2「Existing apps」(任意・links)。 spec § 9 Developer Readiness 表の「Open source / portfolio」行が対応。 確定: pinned 候補は採択前に `google-xr-ops` + `ai-driven-template` を PUBLIC 化して上記 4 つ + (任意) `even-memory-bridge` (G2/R1/Pixel HUD + ライフログ) の構成を予定。
