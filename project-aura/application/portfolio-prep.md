...
- [x] **Step 2: profile README push** → commit `e963410` "docs: profile narrative refresh for current XR project" (GitHub contents API 経由)。 § 2.2 / 2.3 / 2.4 の diff 反映 + **approved 追加修正**: `ai-driven-template` 行を削除 (別 org org へ transfer + private 化済で reviewer には 404 のため。 user 承認 2026-06-06)。 Pinned work は 4 項目に
- [ ] **Step 3 (user 手動のまま、 重要度 UP): pin UI の整理**
  - **実態は想定と乖離**: 実際の pin は 3 つ (`google-xr-ops` + `別 org/sharepoint-rag-azure` (archive badge 付) + `別 org/sharepoint-rag-lite`)。 `even-memory-bridge` が pin されておらず、 会社 org repo が 2 つ並ぶ
...
2. ~~Contributors に `別 org` 混入~~ → **解消 (2026-06-06)**。 repo 削除 → `gh repo create` 再作成 → README を YuhtaIhara 単独 author の 1 commit (`eb90bef`) で push。 履歴から会社 account 消滅
