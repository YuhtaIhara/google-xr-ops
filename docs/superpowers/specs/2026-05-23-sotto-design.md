...
| 関連 | `../../../project-aura/README.md` (Program 概要) / `../../google-xr-design.md` (ops 設計累積) |
| 後段 | 採択判明後 (2026-07-15+) に新 repo `~/work/google/sotto/` を bootstrap、 本 doc が implementation plan の base になる |
| 前身 | even-memory-bridge (G2 用 frozen PJ) の memory + retrieval 知見を engineer 特化 visual HUD に発展 |
...
2. **Even repo 流用 component リスト (`2026-05-23-sotto-even-component-reuse.md`) → PDF** (file-level inventory、 流用率 60-70% の証拠)
3. (optional) architecture diagram を画像 export (本 spec § 4 から)
...
| Open source / portfolio (form Page 2 Existing apps) | https://github.com/YuhtaIhara (pinned: even-memory-bridge + google-xr-ops、 profile README から sharepoint-rag-azure + sharepoint-rag-lite を link) |
...
| Development progress (form Page 3 0-10 scale) | **2** (design + spec 完了 + 前身 PJ memory-bridge で RAG/STT pipeline 完成品所持 + PoC mock HUD 完了 / DP4 emulator の Compose Glimmer 知見) |
...
採択判明と同時に新 repo `~/work/google/sotto/` を個人 workflow template から bootstrap:
