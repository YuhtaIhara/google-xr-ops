...
- ☑ **Productivity** (主軸、 並列調理 conductor = 生産性 tool)
- ☑ **Health & Wellness** (家庭料理 → 健康な食生活への adoption)
- ☑ **Education** (新スパイス / 新料理を覚える学習体験)
...
1. **`2026-05-21-spice-sutra-design.md` → PDF** (本 spec、 11 section / 約 530 行)
2. **`2026-05-21-spice-sutra-catalyst-application.md` → PDF** (plan、 10 task / 約 920 行、 「実行体制まで設計済」 証拠)
3. **(plan Task 6 完了時) PoC screenshot / 動画** (Compose Glimmer mock HUD on DP4 emulator) ← Development progress を 1→2 に押し上げる軸
4. (optional) architecture diagram を画像 export (本 spec § 4 から)
5. (optional) Indian recipe vertical の defense PDF (本 spec § 2 Positioning から抜粋)

PDF 化時の注意: AI 痕跡 (「Claude」 「AI 駆動」 等) を残さない (CLAUDE.md project rule)。 本 spec / plan は既に規約準拠。
...
|---|---|
| Company Name (form Page 1) | [TODO: `delight-x.co.jp` (会社単位) or `YuhtaIhara (Individual)` (indie 単位) — user 判断] |
| Job title (form Page 1) | [TODO: Indie Developer / Founder / Engineer 等 — user 判断] |
| Country (form Page 1) | Japan |
| Region (form Page 1) | APAC |
| Team size (form Page 2 自由記述) | Solo indie developer (`YuhtaIhara` / `delight-x.co.jp`) |
...
| Open source / portfolio (form Page 2 Existing apps) | https://github.com/YuhtaIhara ([TODO: pinned repo 整理]) |
| Concept validation | Wet-hand recipe access is a personal pain validated for years on G2 (HUD only, no camera) |
| Development progress (form Page 3 0-10 scale) | **1** (design + spec 完了、 PoC 未) → plan Task 6 PoC 完了で **2** (PoC 1 画面動作確認済) に bump |

### Funding Requirements

**Request**: **No grant required** (応募 form Page 4 Grant amount で選択)

**理由 (form の自由記述欄が無いため pitch 本文 or 補足で表現)**:

- 応募 form 実物の選択肢は `No grant required` / `$10K` / `$15K` / `$20K` の 4 択 (中間額不可)
- Spice Sutra の MVP cost (Gemini API tuning) は indie 1 人の試算で **年 5-15 万円** 程度に収まる見込み、 $10K (≈ 150 万円) は明確に overshooting
- "Limited grants at each level exist" 文言、 控えめ姿勢 (No grant) は採用率を上げる方向に効く
...
> "We are not requesting grant funding for this application. The MVP scope (5 Indian recipe templates with hybrid timer + camera Gemini vision checks) can be self-funded by a solo developer. We're applying for hardware kit access and technical guidance — these are the multiplicative resources we cannot replicate independently."

### Form Factor Selection (排他、 form Page 3)

応募 form 実物は **Wired XR glasses / Display & Audio glasses の排他 2 択**。 両方なら別 form 2 通提出。

**選択**: ☑ **Display & Audio glasses** (production design prototype)

**根拠**:
- Spice Sutra core (Display HUD + Audio cue + Camera frame 周期 chk) は production form factor (audio glasses + display glasses) と直接 fit
- Wired XR glasses (Aura) は immersive 有線 device、 cooking で wired は実用不可 (調理場を 1 m 動けない)
...
- Aura (Wired XR) 版は将来 v2 以降に「より immersive な Indian cooking experience」 として別 form 出す余地あり (今期は scope 外)

### Tech Stack (form Page 2 multi-select)

応募 form 実物の選択肢 (Android with Compose / Android with Views / Godot / Unity / Unreal / Native OpenXR) に対し:
...
Indian cooking diaspora は世界中 (北米 / 英国 / 中東 / 東南アジア 等)、 vertical 性質上 Global 1 つで十分。 JAPAC 限定にしない。
